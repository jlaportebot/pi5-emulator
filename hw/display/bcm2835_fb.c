...
static void bcm2835_fb_init(Object *obj)
{
    ...
    /* Create the iomem without an owner so we can manually control its container.
     * It will initially be added to the root container, but we'll remove it immediately. */
    /* Use object_get_canonical_path for a unique name to avoid duplicate property errors */
    const char *canonical_path = object_get_canonical_path(obj);
    memory_region_init_io(&s->iomem, NULL, &bcm2835_fb_ops, s, canonical_path,
                          0x10);
    /* Remove from root container immediately */
    if (s->iomem.container) {
        memory_region_del_subregion(s->iomem.container, &s->iomem);
    }
    /* Don't call sysbus_init_mmio - we'll map it manually in the realize function */
...
static void bcm2835_fb_realize(DeviceState *dev, Error **errp)
{
    ...
    fprintf(stderr, "DEBUG: bcm2835_fb_realize called\\n");
    ...
    fprintf(stderr, "DEBUG: got dma-mr link: %p\\n", obj);
...
static const Property bcm2835_fb_props[] = {
    ...
    DEFINE_PROP_LINK("dma-mr", BCM2835FBState, dma_mr,
