...
static void bcm283x_base_init(Object *obj)
{
    ...
    fprintf(stderr, "DEBUG: bcm283x_base_init called for %p, core_count=%d\n", obj, bc->core_count);

    for (n = 0; n < bc->core_count; n++) {
        char name[32];
        snprintf(name, sizeof(name), "cpu[%d]", n);
        fprintf(stderr, "DEBUG: bcm283x_base_init: initializing %s\n", name);
        object_initialize_child(obj, name, &s->cpu[n].core,
                                bc->cpu_type);
    }
    if (bc->core_count > 1) {
